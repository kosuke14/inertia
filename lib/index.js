...
module.exports = class {
	...
	http(req, res, next = () => res.end("")) {
		...
		if (
			req.pathname == "/client_hook" ||
			req.pathname == "/client_hook/"
		)
			return res.end(
				fs.readFileSync(__dirname + "/window.js", "utf-8")
			);
			...
			(proxyResponse) => {
				...
				proxyResponse
					...
					.on("end", () => {
						...
						proxify.html = (body) => {
							const html = new (require("./dom").JSDOM)(
									body,
									{
		...
		if (!res.writableEnded)
			req.on("data", (data) => makeRequest.write(data)).on(
				"end",
				() => makeRequest.end()
			);
	}
				...
				if (
					...
					options.localAddress =
						this.config.localAddress[
							Math.floor(
								Math.random() *
									this.config.localAddress.length
							)
						];
